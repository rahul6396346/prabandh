...
const DeanDashboard = () => {
  const [deanDetails, setDeanDetails] = useState({
    name: "",
    id: "",
    position: "",
    department: "",
  ...
  useEffect(() => {
    ...
    if (currentUser) {
      setDeanDetails({
        ...
        position: currentUser.designation || "Dean",
  ...
  const quickActions = [
    { name: "Academic Affairs", icon: Users, path: "/dean/academics/overview", color: "text-blue-600" },
    { name: "Faculty Coordination", icon: CheckSquare, path: "/dean/faculty/coordination", color: "text-green-600" },
    { name: "Student Affairs", icon: GraduationCap, path: "/dean/students/affairs", color: "text-purple-600" },
    { name: "Curriculum Development", icon: BookOpen, path: "/dean/curriculum/development", color: "text-amber-600" },
    { name: "Academic Reports", icon: BarChart3, path: "/dean/reports/academic", color: "text-cyan-600" },
    { name: "Leave Approvals", icon: CalendarClock, path: "/dean/leave/approvals", color: "text-[#8B0000]" },
    { name: "Quality Assurance", icon: Award, path: "/dean/quality/assurance", color: "text-teal-600" },
    { name: "Academic Calendar", icon: Calendar, path: "/dean/calendar", color: "text-indigo-600" },
  ];
    ...
    <div className="p-6 pb-20 space-y-6 bg-[#f9fafb]">
      ...
      <div className="bg-gradient-to-r from-[#8B0000] to-[#AA0000] rounded-lg shadow-lg p-6 text-white mb-8">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <h1 className="text-2xl font-bold">Welcome, {deanDetails.name}!</h1>
          ...
          <div className="flex items-center gap-3">
            <Badge variant="outline" className="bg-white/10 hover:bg-white/20 text-white border-white/20 flex gap-1 items-center">
              <span className="font-medium">ID: {deanDetails.id}</span>
            </Badge>
            <Badge variant="outline" className="bg-white/10 hover:bg-white/20 text-white border-white/20 flex gap-1 items-center">
              {deanDetails.position}
            </Badge>
          </div>
        </div>
      </div>
      ...
      {/* Dean Profile and Quick Actions */}
          ...
          <CardContent className="pt-6">
            <div className="flex justify-center">
              <div className="h-24 w-24 rounded-full bg-gradient-to-br from-[#8B0000] to-[#AA0000] flex items-center justify-center text-white text-2xl font-bold shadow-lg">
                {deanDetails.name.split(' ').map(n => n[0]).join('')}
              </div>
            </div>
            <div className="mt-6 space-y-3">
              <div className="flex justify-between items-center pb-2 border-b border-gray-100">
                <span className="text-sm font-medium text-gray-500">Name:</span>
                <span className="font-medium">{deanDetails.name}</span>
              </div>
              <div className="flex justify-between items-center pb-2 border-b border-gray-100">
                <span className="text-sm font-medium text-gray-500">ID:</span>
                <span className="font-medium">{deanDetails.id}</span>
              </div>
              <div className="flex justify-between items-center pb-2 border-b border-gray-100">
                <span className="text-sm font-medium text-gray-500">Position:</span>
                <span className="font-medium">{deanDetails.position}</span>
              </div>
              <div className="flex justify-between items-center pb-2 border-b border-gray-100">
                <span className="text-sm font-medium text-gray-500">Department:</span>
                <span className="font-medium">{deanDetails.department}</span>
              </div>
              <div className="flex justify-between items-center pb-2 border-b border-gray-100">
                <span className="text-sm font-medium text-gray-500">Dean Since:</span>
                <span className="font-medium">{deanDetails.since}</span>
              </div>
            </div>
            <div className="mt-6">
        ...
        <Card className="col-span-1 lg:col-span-2 border-0 shadow-md rounded-lg overflow-hidden transform hover:shadow-lg transition-all duration-300 flex flex-col">
          ...
          <CardContent className="pt-6 px-3">
            <div className="max-h-[320px] overflow-y-auto pr-2 space-y-4" style={{ scrollbarWidth: 'thin', scrollbarColor: '#D1D5DB transparent' }}>
              {[
                { id: 1, task: "Academic Council Meeting", time: "10:30 AM - 11:30 AM", location: "Dean Office", description: "Monthly Academic Affairs Review" },
                { id: 2, task: "Faculty Coordination Meeting", time: "01:00 PM - 03:00 PM", location: "Conference Room", description: "Interdepartmental Faculty Meeting" },
                { id: 3, task: "Curriculum Review Session", time: "03:00 PM - 04:00 PM", location: "Committee Room", description: "Academic Program Assessment" },
                { id: 4, task: "Student Affairs Meeting", time: "10:00 AM - 11:00 AM", location: "Dean Office", description: "Student Grievance Discussion" },
                { id: 5, task: "Quality Assurance Review", time: "02:00 PM - 03:00 PM", location: "QA Room", description: "Academic Quality Assessment" },
              ].map((task) => (
              ...
              {[1, 2, 3, 4, 5, 6].map((item) => (
                <div key={item} className="border-b border-gray-100 pb-4 group hover:bg-[#F3E5E5]/20 p-2 rounded-md transition-colors">
                  <div className="flex items-center gap-2">
                    ...
                    <p className="font-medium group-hover:text-[#8B0000] transition-colors">Academic Notice #{item}</p>
                  </div>
                  <p className="text-sm text-gray-600 mt-1">
                    This is a sample notice for the academic affairs office.
                  </p>
