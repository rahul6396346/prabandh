...
import { CheckCircle, Filter, FileText, Clock } from "lucide-react";
...
const ApprovedRequests = () => {
  const [approvedRequests, setApprovedRequests] = useState<LeaveApplication[]>([]);
  const [loading, setLoading] = useState<boolean>(false);
  const [error, setError] = useState<string | null>(null);
  const [selectedApplication, setSelectedApplication] = useState<LeaveApplication | null>(null);

  const fetchApprovedRequests = async (silent: boolean = false) => {
    try {
      ...
      const response = await axios.get('http://localhost:8000/api/faculty/leave/applications/', {
        params: { 
          hod_approvals: 'true',
          status: 'approved,approved_by_hod' // Include both status values
        },
        headers: {
      ...
      console.error('Error fetching approved leave applications:', err);
      if (axios.isAxiosError(err)) {
        if (err.response?.status === 401) {
          setError('Authentication failed. Please log in again.');
          setPollingEnabled(false);
        } else if (err.response?.status === 403) {
          setError('You do not have permission to view HOD applications.');
        } else {
          setError(`Failed to fetch approved applications: ${err.response?.data?.error || err.message || 'Unknown error'}`);
        }
      } else {
        setError('Failed to fetch approved leave applications. Please try again.');
      }
    } finally {
      if (!silent) {
        setLoading(false);
  ...
  const { pollingEnabled, setPollingEnabled, isPolling, manualRefresh } = usePoll({
    callback: fetchApprovedRequests,
    onError: (err) => {
        ...
        <CardContent className="p-0">
          {loading ? (
            <div className="p-8 text-center">
              ...
              <p className="mt-2 text-gray-500">Loading approved applications...</p>
            </div>
          ) : approvedRequests.length === 0 ? (
            <div className="p-8 text-center">
              <FileText className="h-12 w-12 text-gray-300 mx-auto mb-2" />
              <p className="text-gray-500">No approved leave applications yet.</p>
            </div>
          ) : (
            <Table>
              <TableHeader>
              ...
              <TableBody>
                {approvedRequests.map((application) => (
                    ...
                    <TableCell>
                      <Dialog>
                        ...
                        <DialogContent className="max-w-2xl">
                          <DialogHeader>
                            <DialogTitle>Approved Leave Application Details</DialogTitle>
                            <DialogDescription>
                              Review the approved leave application details.
                            </DialogDescription>
                          </DialogHeader>
                          ...
                          {selectedApplication && (
                            <div className="space-y-4 mt-4">
                              <div className="grid grid-cols-2 gap-4">
                                ...
                                <div>
                                  <h4 className="text-sm font-medium text-gray-500">Approved On</h4>
                              ...
                              {selectedApplication.remarks && (
                                <div>
                                  <h4 className="text-sm font-medium text-gray-500">Approval Remarks</h4>
                                  <p className="p-2 bg-gray-50 rounded mt-1">{selectedApplication.remarks}</p>
                                </div>
                              )}
                            </div>
                          )}
