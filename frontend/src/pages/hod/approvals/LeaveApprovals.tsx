...
const LeaveApprovals = () => {
  ...
  const fetchLeaveApplications = async (silent: boolean = false) => {
    try {
      ...
      const response = await axios.get('http://localhost:8000/api/faculty/leave/applications/', {
        params: { 
          hod_approvals: 'true',
          status: 'pending' // Filter to only show applications pending HOD approval
        },
        headers: {
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      await axios.post(`http://localhost:8000/api/faculty/leave/applications/${selectedApplication.id}/hod_approve/`, 
        { remarks },
        {
          headers: {
  ...
  const handleReject = async () => {
    ...
    try {
      ...
      await axios.post(`http://localhost:8000/api/faculty/leave/applications/${selectedApplication.id}/hod_reject/`, 
        { remarks },
        {
          headers: {
            'Content-Type': 'application/json',
  ...
  const handleRecommendToDean = async () => {
    if (!selectedApplication) return;
    ...
    try {
      ...
      await axios.post(`http://localhost:8000/api/faculty/leave/applications/${selectedApplication.id}/hod_recommend_to_dean/`, 
        { remarks },
        {
          headers: {
            'Content-Type': 'application/json',
      ...
      setActionSuccess('Leave application recommended to Dean successfully');
      // Refresh the list
      fetchLeaveApplications();
      // Reset states
      setSelectedApplication(null);
      ...
      console.error('Error recommending to Dean:', err);
      if (axios.isAxiosError(err)) {
        if (err.response?.status === 401) {
          setError('Authentication failed. Please log in again.');
        } else {
          setError(`Failed to recommend to Dean: ${err.response?.data?.error || err.message || 'Unknown error'}`);
        }
      } else {
        setError('Failed to recommend leave application to Dean. Please try again.');
      }
    }
  };
  ...
  const handleRecommendToVC = async () => {
    ...
    try {
      ...
      await axios.post(`http://localhost:8000/api/faculty/leave/applications/${selectedApplication.id}/hod_recommend_to_vc/`, 
        { remarks },
        {
          headers: {
            'Content-Type': 'application/json',
      ...
      <Card>
        <CardHeader className="bg-muted/30">
          <CardTitle className="text-lg flex items-center gap-2">
            ...
            Leave Applications Pending HOD Approval
          </CardTitle>
        </CardHeader>
        <CardContent className="p-0">
          {loading ? (
            ...
            <div className="p-8 text-center">
              ...
              <p className="text-gray-500">No leave applications pending HOD approval yet.</p>
            </div>
          ) : (
            <Table>
              <TableHeader>
