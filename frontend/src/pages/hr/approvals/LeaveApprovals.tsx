  ...
  const fetchLeaveApplications = async (silent: boolean = false) => {
    try {
      ...
      const response = await axios.get('http://localhost:8000/api/faculty/leave/applications/', {
        params: { 
          hr_approvals: 'true',
          status: 'forwarded_to_hr' // Filter to only show applications forwarded to HR
        },
        headers: {
      ...
      <Card>
        <CardHeader className="bg-muted/30">
          <CardTitle className="text-lg flex items-center gap-2">
            ...
            Leave Applications Forwarded to HR
          </CardTitle>
        </CardHeader>
        <CardContent className="p-0">
          {loading ? (
            ...
            <div className="p-8 text-center">
              ...
              <p className="text-gray-500">No leave applications forwarded to HR yet.</p>
            </div>
          ) : (
            <Table>
              <TableHeader>
