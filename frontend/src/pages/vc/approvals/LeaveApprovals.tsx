...
import { Clock, CheckCircle, XCircle, AlertCircle, Filter, FileText, ChevronDown } from "lucide-react";
...
const LeaveApprovals = () => {
  ...
  const fetchLeaveApplications = async (silent: boolean = false) => {
    try {
      ...
      const response = await axios.get('http://localhost:8000/api/faculty/leave/applications/', {
        params: { 
          vc_approvals: 'true',
          status: 'forwarded_to_vc' // Filter to only show applications forwarded to VC
        },
        headers: {
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      await axios.post(`http://localhost:8000/api/faculty/leave/applications/${selectedApplication.id}/vc_approve/`, 
        { remarks },
        {
          headers: {
  ...
  const handleReject = async () => {
    ...
    try {
      ...
      await axios.post(`http://localhost:8000/api/faculty/leave/applications/${selectedApplication.id}/vc_reject/`, 
        { remarks },
        {
          headers: {
            'Content-Type': 'application/json',
      ...
      <Card>
        <CardHeader className="bg-muted/30">
          <CardTitle className="text-lg flex items-center gap-2">
            ...
            Leave Applications Pending VC Approval
          </CardTitle>
        </CardHeader>
        <CardContent className="p-0">
          {loading ? (
            ...
            <div className="p-8 text-center">
              ...
              <p className="text-gray-500">No leave applications pending VC approval yet.</p>
            </div>
          ) : (
            <Table>
              <TableHeader>
